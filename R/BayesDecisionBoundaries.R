BayesDecisionBoundaries <- function(Means, SDs, Weights, IsLogDistribution,
                                    MinData, MaxData, Ycoor = F){
  ...
  for(i in 1:length(Means)){                # If there are equal means, order them regarding their SD
    EqIdx = which(Means == Means[i])
    if(length(EqIdx) > 1){
      NewInternalOrder = order(SDs[EqIdx])
      SDs[EqIdx] = SDs[EqIdx][NewInternalOrder]
      Weights[EqIdx] = Weights[EqIdx][NewInternalOrder]
    }
  }
  ...
  for(i in 1:L1){
  	i1 = i+1
    Decision = Intersect2Mixtures(Means[i],  SDs[i],  Weights[i],
                                  Means[i1], SDs[i1], Weights[i1],
                                  IsLogDistribution[i:i1])
