...
for( i in 1:AnzMixtures){
	if(IsLogDistribution[i] ==1){ # Log Normal
		PDataGivenClass[,i] <- symlognpdf(Kernels,Means[i],SDs[i])
	}else{
...
PClassGivenData <- PClassGivenData / (NormalizationFactor %*% ones(1,2))
...
if ( PlotIt == 1){
  ...
  for (i in 1:AnzMixtures){
    ...
    gPlot = gPlot + geom_line(data = df, aes(x=x,y=y), color="blue")
  }
  
  df = data.frame(x = Kernels, y=Posteriors[,1])
  gPlot = gPlot + geom_line(data = df, aes(x=x,y=y), color="green")
  df = data.frame(x = Kernels, y=Posteriors[,2])
  gPlot = gPlot + geom_line(data = df, aes(x=x,y=y), color="red")
