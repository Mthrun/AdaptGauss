AdaptLGL = function(Data,Means=NaN,SDs=NaN,Weights=NaN,ParetoRadius=NaN,LB=NaN,HB=NaN,ListOfAdaptGauss){
  #    Out=AdaptLGL(Data,Means,Deviations,Weights,ParetoRadius,LowErrBound,HiErrBound,LB,HB);
  ...
  #    Deviations(1:c)                The sdev   of the distributions            default: nanstd(Data)
  ...
  #   
  #    OUTPUT
  #    Out$M(1:L)                     The adapted means of the distributions
  #    Out$S(1:L)                     The adapted sdev  of the distributions              
  #    Out$W(1:L)                     The adapted weights of the distributions
  ...
  outputApp=runApp(list(
    ...
    server = function(input, output, session){
      ...
      if (length(data)>10000){
        data <- data[rsampleAdaptLGL(10000,length(data))];
      ...
      observe({
        ...
        if (input$expMax>0){
          Var <- EM_LGL(data,GM,GS,GW,numIterations)
          GM <<- Var$EMmean
          GS <<- Var$EMsdev
          GW <<- Var$EMalphas
        }
      ...
      output$PDE <- renderPlot({
        ...
        plot(Kernels, ParetoDensity,xlim=xlimit,ylim=ylimit, col="black", axes = FALSE, xlab = "x", , ylab = "ParetoDensity", type="l", lwd=3)
        ...
        sig=seq(0,0,length.out=3)
        mu=seq(0,0,length.out=3) 
        #GMLGL=GM
        sig[1] = sqrt(log(GS[1]*GS[1]/(GM[1]*GM[1])+1))
        sig[2] = GS[2]
        sig[3] = sqrt(log(GS[3]*GS[3]/(GM[3]*GM[3])+1)) 
        if (GM[1]<=0){
          mu[1] = (log(-GM[1])-0.5*sig[1]*sig[1])
        } else { 
          mu[1] = (log(GM[1])-0.5*sig[1]*sig[1])
        }
        mu[2] =  GM[2]
        if (GM[3]>=0){      
          mu[3] = (log( GM[3])-0.5*sig[3]*sig[3])
        } else {
          mu[3] = (log(-GM[3])-0.5*sig[3]*sig[3])
        ...
        for (i in 1:numGauss){
          if (i==1) {
            if (GM[i]<=0){
              F=dlnorm(-(Kernels),mu[i],sig[i])*GW[i]
            } else {
              F=dlnorm((Kernels),mu[i],sig[i])*GW[i]
            }
          }
          if (i==2){
            F=dnorm(Kernels,mu[i],sig[i])*GW[i]
          }
          if (i==3) {
            if (GM[i]>=0){
              F=dlnorm(Kernels,mu[i],sig[i])*GW[i]
          ...
          if (input$showComponents){
            if (i==currGauss) {         # plotte current gaussian in Gr?n
              colPlot="green"
            } else{
              colPlot="blue"
            }
            points(Kernels, F,xlim=xlimit,ylim=ylimit, col=colPlot, type="l", lwd=3)
            par(new = TRUE) # der befehl das der n?chste den alten nicht ?bermalt
          }
        }
        ...
        if (input$CloseButton+input$PlotFig > 0){
          ...
          for (i in 1:numGauss){
            ...
            if (isolate(input$showComponents)){
              colPlot="blue"
              plot(Kernels, F,xlim=xlimit,ylim=ylimit, col=colPlot, axes = FALSE, xlab = " ", , ylab = " ", type="l", lwd=3)
              par(new = TRUE) # der befehl das der n?chste den alten nicht ?bermalt
            }
          }   
          # Plotte Summe ?ber alle gaussians (in Rot)
          plot(Kernels, FSum,xlim=xlimit,ylim=ylimit, col="red", axes = FALSE, xlab = " ", , ylab = " ", type="l", lwd=3)
          if (isolate(input$showBayes)){
            for (i in 1:length(BB)){
              par(new = TRUE) # der befehl das der n?chste den alten nicht ?bermalt
